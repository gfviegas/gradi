...
defmodule GradiWeb.MoviesController do
  ...
  defp insert_languages(%{languages: languages}), do: Enum.map languages, &Gradi.Languages.upsert_language/1
  defp insert_companies(%{companies: companies}), do: Enum.map companies, &Gradi.Companies.upsert_company/1
  defp insert_directors(%{directors: directors}), do: Enum.map directors, &Gradi.Directors.upsert_director/1
  defp insert_actors_and_characters(%{characters: characters, movies: movies}) do
    movies_characters = Enum.map(movies, fn m -> [{m.imdb_id, m.characters}] end) |> List.flatten
  ...
  defp insert_actor_and_character(character, movies_characters) do
    ...
    movie = Enum.find(movies_characters, fn m -> Enum.find_value(elem(m, 1), fn name -> name == character.character end) end)
    IO.inspect movie
    movie_imdb = movie |> elem(0)
    movie = Gradi.Movies.get_by_imdb_id movie_imdb
    Ecto.build_assoc(movie, :characters, %{character: character.character, protagonist: character.protagonist}) |> Repo.insert
  end
